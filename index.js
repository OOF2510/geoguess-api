...
app.get('/getImage', async (req, res) => {
  try {
    if (!process.env.MAP_API_KEY) {
      return res.status(500).json({ error: "Mapillary access token missing in environment variables." });
    }
    ...
    if (imageCache.length > 0) {
      ...
      if (!img || !img.url || !img.coord) {
        return res.status(500).json({ error: "Could not fetch a random image right now. Please try again." });
      }
      const { lat, lon } = img.coord;
